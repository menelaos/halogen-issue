...
busEventSource
  ∷ ∀ f m a r eff
  . MonadAff (avar ∷ AVAR, console :: CONSOLE, random :: RANDOM | eff) m
...
affEventSource
  ...
  . MonadAff (avar ∷ AVAR, console :: CONSOLE, random :: RANDOM | eff) m
  ⇒ (a → f ES.SubscribeStatus)
  → Aff (avar ∷ AVAR, console :: CONSOLE, random :: RANDOM | eff) a
  → ES.EventSource f m
affEventSource k source = ES.EventSource do
  breaker ← liftAff makeVar
  randomNumber <- liftEff $ random
  let
    producer = ES.produceAff \emit → liftAff do
...
--    done = liftAff (EventLoop.break' =<< takeVar breaker)
      ...
      log $ "Done " <> show randomNumber
      (EventLoop.break' =<< takeVar breaker)
  pure { producer, done }
